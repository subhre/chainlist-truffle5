...
contract('ChainList', function(accounts) {
    ...
    it("should throw an exception if you try to buy an article when there is none for sale yet", async () => {
        try {
            ...
            assert.equal(error.reason, "Article should be available for sale");
            //assert(true);
        }
    ...
    it("should throw an exception if you try to buy an article that does not exist", async () => {
        ...
        try {
            await chainListInstance.buyArticle(articleId + 1, {
                from: seller,
                value: web3.utils.toWei(articlePrice, "ether")
            });
            assert.fail();
